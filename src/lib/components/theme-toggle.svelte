...
<label class="swap swap-rotate">
	<input
		...
		checked={docState.theme === 'light'}
		onclick={() => {
			docState.theme = docState.theme === 'dark' ? 'light' : 'dark';
		}}
		aria-label="toggle theme"
		indeterminate={!['light', 'dark'].includes(docState.theme)}
	/>
	...
	<svg
		...
		class="lucide lucide-sparkles swap-indeterminate size-6"
		><path
